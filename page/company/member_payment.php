...
$today = date("Y-m-d");
//추가 구매 확인
$sql = "select count(*) as cnt from `cmap_payments` where mb_id = '{$mb_id}' and '{$today}' BETWEEN payment_start_date and payment_end_date and order_cancel = 0";
$pays = sql_fetch($sql);
...
if($pays["cnt"]>0){//구매연장
    ...
    //최근 구매내역에서 종료일 체크
    $sql = "select * from `cmap_payments` where mb_id = '{$mb_id}' and order_cancel = 0 order by payment_date desc limit 0, 1";
    ...
    $sql = "insert into `cmap_payments` set mb_id ='{$mb_id}', order_type = '{$payment_type}', order_payment_type = '{$order_type}',order_id='{$merchant_uid}', order_amount = '{$amount}', payment_date = now(), payment_time = now(), payment_month = '{$payment_month}',payment_start_date = '{$start_date}', payment_end_date = '{$end_date}'";
