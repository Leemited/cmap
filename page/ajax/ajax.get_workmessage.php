...
$sql = "select * from `cmap_construct_work_msg` where (read_mb_id = '{$member["mb_id"]}' or send_mb_id = '{$member["mb_id"]}') and read_status = 0";
$res = sql_query($sql);
...
<div class="work_list_btns">
    <input type="button" value="업무연락서 목록" onclick="location.href=g5_url+'/page/mypage/my_message_list?const_id=<?php echo $const_id;?>'">
</div>
<table>
    <colgroup>
        <col width="15%">
    ...
    if(count($wlist)!=0){
        for($i=0;$i<count($wlist);$i++) {
            ...
            ?>
            <tr id="msg_<?php echo $wlist[$i]["id"]; ?>" style="cursor:pointer" onclick="location.href=g5_url+'/page/mypage/my_message_list?msg_id=<?php echo $wlist[$i]["id"];?>'">
