...
if($depth==1) {
    ...
    $sql = "delete from `cmap_depth1` where pk_id = '{$pk_id}'";
    sql_query($sql);
...
if($depth==2){
    $sql = "select * from  `cmap_depth2` where pk_id = '{$pk_id}'";
    $item = sql_fetch($sql);
    ...
    $sql = "delete from `cmap_depth2` where pk_id = '{$pk_id}'";
    sql_query($sql);
...
if($depth==3){
    $sql = "select * from  `cmap_depth3` where pk_id = '{$pk_id}'";
    $item = sql_fetch($sql);

    $sql = "select COUNT(*)as cnt from `cmap_depth3` where depth2_id = '{$item['depth2_id']}'";
    $count = sql_fetch($sql);
    if($count["cnt"]<=1){
        alert("상세 항목이 하나 일경우는 상위 목록을 삭제해 주세요".$sql);
        return false;
    }

    $sql = "delete from `cmap_depth3` where pk_id = '{$pk_id}'";
    sql_query($sql);
...
if($depth==4){
    $sql = "select * from  `cmap_depth4` where pk_id = '{$pk_id}'";
    $item = sql_fetch($sql);
    ...
    $sql = "delete from `cmap_depth4` where pk_id = '{$pk_id}'";
    sql_query($sql);
...
if($depth==5) {
    ...
    if($count["cnt"]<=1){
        ...
        $sql = "delete from `cmap_content` where pk_id = '{$pk_id}'";
        sql_query($sql);
