    ...
    <section>
        ...
        <div class="admin_content">
            <div class="desc">
                ...
                <div class="inserts">
                    <form action="<?php echo G5_URL?>/admin/evaluation_insert3.php" method="post" name="insert_form" enctype="multipart/form-data">
            ...
            <div class="edit_content">
                <table id="edit_table" class="resizable">
                    ...
                    <tr id="depth1<?php echo $list[$i]['id'];?>" class="depth<?php if($list[$i]['cnt'] == 1){echo " finish_".$list[$i]["id"];}?>" >
                        ...
                        <!--<tr>-->
                        <?php //}?>
                        <td rowspan="<?php if($list[$i]['depth2'][$j]['depth3'][$k]['depth4'][$l]['cnt']>1){echo $list[$i]['depth2'][$j]['depth3'][$k]['depth4'][$l]['cnt'];}?>" id="depth4_<?php echo $list[$i]['depth2'][$j]['depth3'][$k]['depth4'][$l]['id'];?>">
                        ...
                        for ($m = 0; $m < count($list[$i]['depth2'][$j]['depth3'][$k]['depth4'][$l]['depth5']); $m++) {
                            ...
                            if(count($eval)>2) {
                                for ($c = 0; $c < count($eval); $c++) {
                                    if ($eval[$c] != "") {
                                        echo "<td class='td_center block_" . $c . "' colspan='" . $span[$c] . "'><input type='text' value='" . $eval[$c] . "' onkeyup=\"fnUpdateEval('" . $list[$i]['depth2'][$j]['depth3'][$k]['depth4'][$l]['depth5'][$m]['pk_id'] . "','".$c."',this.value)\"></td>";
                                    }
                                }
                            }else{
...
<script>
    ...
    var upText2 = null;
    function fnUpdateEval(pk_id,index,eval){
        console.log(pk_id+"//"+index+"//"+eval);
        if(!pk_id){
            alert("선택항목의 정보가 올바르지 않습니다.");
            return false;
        }
        ...
        upText2 = setTimeout(function(){
            $.ajax({
                url:g5_url+"/admin/ajax.eval_update.php",
                type:"POST",
                data:{pk_id:pk_id,text:eval,index:index}
            }).done(function(data){
                console.log(data);
            })
        },1000)
