...
//print_r2($data->sheets);

error_reporting(E_ALL ^ E_NOTICE);

$cnt = count($data->sheets);

$filename = explode(".",$_FILES['insert_file']["name"]);

if($cnt > 1) {
    for ($j=0;$j<$cnt;$j++) {
        for ($i = 2; $i <= $data->sheets[$j]['numRows']; $i++) {
            $all['sub_menu'][] = $filename[0];
            ...
            $all['depth5'][] = $data->sheets[$j]['cells'][$i][5]."``".$data->sheets[$j]['cells'][$i][6]."``".$data->sheets[$j]['cells'][$i][7]."``".$data->sheets[$j]['cells'][$i][8]."``".$data->sheets[$j]['cells'][$i][9];
            ...
            if($data->sheets[$j]['cells'][$i][9]){
                if($data->sheets[$j]["cellsInfo"][$i][9]) {
                    $colchk .= "``".$data->sheets[$j]["cellsInfo"][$i][9]["colspan"];
                }else {
                    $colchk .= "``1";
                }
            }
    ...
    for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
        ...
        $all['depth5'][] = $data->sheets[0]['cells'][$i][5]."``".$data->sheets[0]['cells'][$i][6]."``".$data->sheets[0]['cells'][$i][7]."``".$data->sheets[0]['cells'][$i][8]."``".$data->sheets[0]['cells'][$i][9];
        ...
        if($data->sheets[0]['cells'][$i][9]){
            if($data->sheets[0]["cellsInfo"][$i][9]) {
                $colchk .= "``".$data->sheets[0]["cellsInfo"][$i][9]["colspan"];
            }else {
                $colchk .= "``1";
            }
        }
...
for($i=0;$i<count($all["depth1"]);$i++){
    ...
    if($all['depth5'][$i]!=""){
        ...
        $menu_name = " , sub_menu = '{$all["sub_menu"][$i]}'";
        ...
        if(count($chk)>0){
            ...
            $sql = "insert into `cmap_content` set content = '{$all['depth5'][$i]}', depth1_id = '{$first_id}', depth2_id = '{$depth2_max_id}', depth3_id = '{$depth3_max_id}', depth4_id = '{$depth4_max_id}' , id = {$depth5_max_id}, span = '{$all['depth5_info'][$i]}' {$menu_name}";
            sql_query($sql);
        }
    }
}
