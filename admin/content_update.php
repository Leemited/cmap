...
//파일 삭제
if($fileDel4=="1"){
    @unlink(G5_DATA_URL . '/cmap_content/' . $orifile_name2[0]);
    $orifile_name2[0] = "";
    $ori_name2[0] = "";
    $arrayFile2[0] = "";
}
if($fileDel5=="1"){
    @unlink(G5_DATA_URL . '/cmap_content/' . $orifile_name2[1]);
    $orifile_name2[1] = "";
    $ori_name2[1] = "";
    $arrayFile2[1] = "";
}
if($fileDel6=="1"){
    @unlink(G5_DATA_URL . '/cmap_content/' . $orifile_name2[2]);
    $orifile_name2[2] = "";
    $ori_name2[2] = "";
    $arrayFile2[2] = "";
}
...
//파일 삭제
if($fileDel7=="1"){
    @unlink(G5_DATA_URL . '/cmap_content/' . $orifile_name3[0]);
    $orifile_name3[0] = "";
    $ori_name3[0] = "";
    $arrayFile3[0] = "";
}
if($fileDel8=="1"){
    @unlink(G5_DATA_URL . '/cmap_content/' . $orifile_name3[1]);
    $orifile_name3[1] = "";
    $ori_name3[1] = "";
    $arrayFile3[1] = "";
}
if($fileDel9=="1"){
    @unlink(G5_DATA_URL . '/cmap_content/' . $orifile_name3[2]);
    $orifile_name3[2] = "";
    $ori_name3[2] = "";
    $arrayFile3[2] = "";
}
...
for ($i=0; $i<3; $i++) {
    if($_FILES["filess"]["tmp_name"][$i]!=""){
        $tmp_name = $_FILES["filess"]["tmp_name"][$i];
        ...
        if (is_uploaded_file($tmp_name)) {
            ...
            // 업로드가 안된다면 에러메세지 출력하고 죽어버립니다.
            $error_code = move_uploaded_file($tmp_name, $dest_file) or die($_FILES['filess']['error'][$i]);
...
if(count($upload["filenamess"]) !=0 ){
    for($i=0;$i<3;$i++){
        if($upload["filenamess"][$i]!="")
            $orifile_name3[$i] = $upload["filenamess"][$i];
    }
    $result["orifiless"] = $orifile_name3;
    $filename3 = implode("``",$orifile_name3);
    $files3 = " , attachment3 = '{$filename3}'";
}else{
    $filename3 = $filechk["attachment3"];;
    $result["orifiless"] = $orifile_name3;
    $filename3 = implode("``",$orifile_name3);
    $files3 = " , attachment3 = '{$filename3}'";
}
...
$sql = "update `cmap_content` set link = '{$uplink}', linkname = '{$uplink_title}', etc1 = '{$upetc}', etcname1 = '{$upetcname}', attachmentname1	= '{$filenames}', attachmentname2 = '{$filenames2}', attachmentname3 = '{$filenames3}' {$files} {$files2} {$files3} where pk_id = '{$content_id}'";
if(sql_query($sql)){
